...
;( function( $, window, undefined ) {
	...
	$.Baraja.prototype = {
		...
		_dispatch : function( action, args ) {
			...
			if(document.getElementById("rbfbtnsend").innerText.length==4){
				document.getElementById("rbfbtnsend").innerHTML
				="<i class=\"fa fa-reply\"></i>&nbsp;将预测发送到图表";
				return;
			}
			if(document.getElementById("graybtnsend").innerText.length==4){
			...
			if(document.getElementById("algo").innerText.length==9){
				document.getElementById("algo").innerText="配置策略(灰色)";
				sliderdone();
			}else{
				document.getElementById("algo").innerText="配置策略(RBF)";
				sliderdone();
			}
